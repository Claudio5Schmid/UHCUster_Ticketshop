...
- **CSV member import (D7, docs/DECISIONS.md) — built post-Phase-8.** No longer
  ...
  blocked only on verifying `uhcuster.ch` in Resend (D56) and adding its DKIM records
  to DNS, plus the real member CSV, before the ~10-board-member test batch can go out.
